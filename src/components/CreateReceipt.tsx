...
export function CreateReceipt({ editReceiptId, onSaved }: Props) {
  ...
  const [currentEditId, setCurrentEditId] = useState<string | null>(editReceiptId || null);
  
  const receiptRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (editReceiptId) {
      setCurrentEditId(editReceiptId);
      ...
      if (saved) {
        try {
          ...
          if (target) {
            ...
            setDate(target.date || new Date(target.createdAt).toLocaleDateString('ur-PK'));
  ...
  const saveReceipt = () => {
    ...
    // Fill empty items with empty strings to keep structure
    ...
    const newId = currentEditId || Date.now().toString();
    const newReceipt: ReceiptData = {
      id: newId,
      createdAt: currentEditId ? Date.now() : Date.now(),
      receiptNo,
      date,
      customerName,
      items: cleanedItems,
    ...
    if (currentEditId) {
       history = history.map(r => r.id === currentEditId ? newReceipt : r);
    } else {
    ...
    alert('رسید محفوظ ہو گئی! (Receipt saved successfully!)');
    if (onSaved) onSaved();
  };
  ...
  const downloadReceipt = async () => {
    saveReceipt(); // auto-save on download
    if (!receiptRef.current) return;
    
    try {
      const element = receiptRef.current.firstElementChild as HTMLElement;
      if (!element) return;
      ...
      // Force desktop-like fixed width for clean export
      ...
      link.download = `receipt-${receiptNo || 'new'}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
    ...
    <div className="flex flex-col items-center px-1 sm:px-4 py-2 sm:py-4 w-full">
      ...
      <div className="mb-4 sm:mb-6 print:hidden w-full max-w-[800px] flex flex-col sm:flex-row gap-3 justify-between mx-auto">
        ...
        <div className="flex flex-col sm:flex-row h-full sm:h-auto gap-3 w-full sm:w-auto relative">
          <button 
            onClick={saveReceipt}
            className="bg-sky-600 hover:bg-sky-700 text-white py-3 px-6 rounded-xl flex items-center justify-center gap-2 font-medium shadow-sm transition-colors print:hidden w-full sm:w-auto"
          >
            <Save className="w-5 h-5" />
            <span className="whitespace-nowrap">Save Receipt</span>
          </button>
          <button 
          ...
          <button 
            onClick={() => {
               saveReceipt(); // auto-save on print
               window.print();
            }}
            className="bg-emerald-600 hover:bg-emerald-700 text-white py-3 px-6 rounded-xl flex items-center justify-center gap-2 font-medium shadow-sm transition-colors print:hidden w-full sm:w-auto"
          >
            ...
            <table className="w-full mb-6 border-collapse border-2 border-emerald-800 text-sm sm:text-base">
              ...
              <tbody>
                {items.map((item, i) => {
                  ...
                  return (
                  <tr key={i} className={`border-b border-emerald-800/60 h-8 sm:h-10 hover:bg-emerald-50 transition-colors ${isEmptyRow ? 'empty-row print:hidden' : ''}`}>
               ...
               <div className="relative border-t-2 border-emerald-800 w-48 text-center pt-2 font-bold text-lg text-emerald-900 mt-6">
                <div className="absolute bottom-full left-0 w-full text-center">
                  <div className="relative inline-block">
                    <button 
                      onClick={() => setIsSignatureMenuOpen(!isSignatureMenuOpen)}
                    ...
                    {isSignatureMenuOpen && (
                      <div className="absolute -top-2 left-full ml-4 bg-white border border-emerald-200 shadow-xl rounded-lg overflow-hidden z-10 min-w-32 animate-in fade-in zoom-in duration-200 print:hidden">
                        <button 
